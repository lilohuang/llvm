...
and `architecture::intel_gpu_cri`.
...
`architecture::intel_gpu_ptl_u`, `architecture::intel_gpu_wcl`,
`architecture::intel_gpu_nvl_s`, `architecture::intel_gpu_nvl_u`,
`architecture::intel_gpu_nvl_p`, and `architecture::intel_gpu_cri`
...
IMPORTANT: The supported instructions may be run on GFX90A (MI200, MI210,
MI250 and MI250X GPUs) or the GFX1100, GFX1101 and GFX1102 RDNA 3
architectures. When compiling for the `ext_oneapi_hip` backend, the matching
target arch flag, such as `-fsycl-targets=amd_gpu_gfx90a` or
`-fsycl-targets=amd_gpu_gfx1102`, must be used. An attempt to run the compiled
code on an unsupported architecture will throw an error.


[frame="none",options="header"]
|======================
| Architecture | A and B type | C and D type | M | N | K
.7+| GFX90A .2+| `matrix_type::fp16`  .2+| `matrix_type::fp32`
|32 |32 |8 
|16 |16 |16
...
|16 |16 |4
.3+| GFX1100, GFX1101, GFX1102
| `matrix_type::fp16` | `matrix_type::fp32` |16 |16 |16
| `matrix_type::bf16` | `matrix_type::fp32` |16 |16 |16
| `matrix_type::sint8` | `matrix_type::sint32` |16 |16 |16
|======================
